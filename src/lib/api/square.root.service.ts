...
export const squareRootService = async (variables: any): Promise<InternalResponseInterface | null> => {
    let queryParams = "";
    const sources = apiSources["v1"];
    const { numberA, operationId } = variables
    queryParams = `?${new URLSearchParams({ numberA, operationId })}`
    const baseUrl = `${sources.base_url}${sources.endpoints.squareRoot.name}${queryParams}`
